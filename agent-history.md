...
## 2026-05-28 — Add client_id multi-tenancy scoping across all tables + pipeline

**What changed:** Threaded an explicit `client_id` (default `'default'`) through the entire stack.
- `storage/db.py`: added `client_id TEXT NOT NULL DEFAULT 'default'` to all 5 tables (`ads`, `analysis_results`, `brand_dna`, `jobs`, `intel_signals`); widened the ads uniqueness to `UNIQUE(ad_id, advertiser, platform, client_id)`. Every write (`save_ads`, `save_analysis`, `save_brand_dna`, `update_ad_type`, `update_ad_vision`, `create_job`, `save_intel_signal`) and every read (`get_brand_dna`, `get_latest_intel_signal`, `get_ads`, `get_ads_platforms`, `get_latest_analysis`, `get_stale_market_context`) now takes/filters by `client_id`. `migrate_columns()` adds `client_id` to all 5 tables (Postgres `ADD COLUMN IF NOT EXISTS` with DEFAULT backfills existing rows; SQLite PRAGMA-guarded ADD COLUMN) and, on Postgres, drops the old `ads_ad_id_advertiser_platform_key` constraint and creates the client-scoped unique index `ads_uniq_client`.
- `analysis/agent.py`: `run_all_passes(..., client_id="default")` → passes it to the `get_stale_market_context` fallback.
- `main.py`: `run(..., client_id="default")` threads it to all DB calls; new `--client-id` CLI flag; client shown in the run panel.
- `worker.py`: `run_pipeline(..., client_id="default")` → `pipeline.run`; `startup()` now also calls `migrate_columns()`.
- `api.py`: `AnalyzeRequest.client_id` (body, default 'default'); `JobResponse.client_id`; `GET /brand-dna`, `GET /intel-signal`, `POST /regen` take `?client_id=`; `GET /jobs/{id}` scopes result reads by the job's stored `client_id`; lifespan now calls `migrate_columns()`.
- `regen_dna.py`: raw-SQL helpers + `db.get_ads` scoped to `client_id` (default 'default').
**Why:** Production gap #3 / Next Step #3 — the system was single-tenant. Every table keyed only on `advertiser`, so two clients analyzing the same advertiser collided destructively (`save_ads` DELETEs by advertiser+platform; `save_brand_dna` DELETEs by advertiser). CLAUDE.md mandates every table carry `client_id` and every query be client-scoped. Decisions (user): explicit `client_id` field in requests (not API-key-derived yet); backfill existing data to `'default'` for backward compatibility.
**How it helps:** Two clients can now analyze the same advertiser without clobbering each other's ads/analysis/Brand DNA. Existing prod data (308 ads, 36 analysis, 3 brand_dna, 4 jobs, 1 intel_signal) backfills to `client_id='default'` and stays readable via the default client. CLI and existing callers work unchanged (default applies).
**Traceback notes:** Verified locally on throwaway SQLite — isolation tests (two clients same advertiser, scoped overwrite, unknown-client-sees-nothing) and migration tests (old schema → backfill → idempotent) both green. **Critical for rollout:** `init_db()` uses `CREATE TABLE IF NOT EXISTS` and will NOT add `client_id` to existing prod tables — `migrate_columns()` is the only path that does, so it's now called on both API lifespan and worker startup (previously only inside `main.run()`). Prod ads UNIQUE constraint name confirmed as `ads_ad_id_advertiser_platform_key` (matches the migration's DROP). SQLite can't ALTER a UNIQUE constraint in place, so existing local dev DBs keep the old (ad_id, advertiser, platform) uniqueness — only fresh local DBs get the client-scoped one; this is fine for single-client dev. If a prod query errors with "column client_id does not exist," migrate_columns didn't run — check API/worker startup logs.
**Shipped + verified live (2026-05-28):** commit `d640f26` deployed to both Railway services. Prod migration ran on startup — all 5 tables gained `client_id`, existing rows (308/36/3/4/1) backfilled to `'default'`, old `ads_ad_id_advertiser_platform_key` constraint dropped, `ads_uniq_client` index created. Two-client smoke test passed: ran HubSpot under `client_id=tenantB` while existing HubSpot data sat under `default` — afterward `default` DNA was untouched (created_at still 2026-05-27T22:22:25, 16302 chars) and `tenantB` had its own new DNA (2026-05-28, 13070 chars); ads split 99 (default) / 94 (tenantB), brand_dna + analysis isolated 1/1 and 9/9. No collision. Deploy gotcha: `serviceInstanceDeployV2` with no `commitSha` re-runs the last cached build (silently shipped stale `284320b`/`b40566b` first, so the migration didn't run) — always pass `commitSha` to force a fresh build of a new push.

## 2026-05-28 — Fix Railway deploy-source wiring + verify gemini-3.5-flash end-to-end

**What changed:** No source files in `intelligence-agent/` changed — this was a Railway service-config fix (via GraphQL `serviceInstanceUpdate`) plus an end-to-end verification run. (1) Set `dockerfilePath = "Dockerfile"` on both `intelligence-api` and `intelligence-worker` service instances. (2) Repointed `intelligence-api` source repo from `rishinagarajan-diffai/marketing-intelligence-agent` → `rishinagarajan-diffai/intelligence-agent`. (3) Redeployed both. Docs updated: `current-state.md` (new "Deploy-source wiring fix" section, Next Steps #1 marked done, Asana added to Last Successful Runs).
**Why:** The hosted API was returning 502 (crash-looping on `/start.sh: No such file or directory`). Two compounding causes: the API service was deploying from the **wrong repo** (the deprecated `marketing-intelligence-agent`, pinned at stale Ollama-era commit `1d846b0` with no Dockerfile), and both services had drifted to the `RAILPACK` builder which ignored the Dockerfile. Yesterday's `GEMINI_MODEL` env change triggered a git redeploy from the stale repo, regressing the service. This blocked the deferred gemini-3.5-flash smoke test.
**How it helps:** Hosted API is live again (`/docs` 200). Both services now build the current Dockerfile from the correct repo. gemini-3.5-flash is **verified end-to-end**: Asana via `POST /analyze` completed in 285s — 148 ads, 15,616-char Brand DNA, all 11 sections, on-brand voice fingerprint + synthetic templates, no structural drift. The 05-28 model bump is confirmed safe to keep.
**Traceback notes:** Railway's `Builder` GraphQL enum has only HEROKU/NIXPACKS/PAKETO/RAILPACK — **there is no `DOCKERFILE` value**; forcing a Dockerfile build is done by setting `dockerfilePath`, not `builder`. A service-level `builder = RAILPACK` overrides `railway.toml`'s `builder = "DOCKERFILE"`. The local `origin` remote still points at the deprecated `marketing-intelligence-agent`; canonical repo is the `public` remote (`intelligence-agent`) — local `main` was 18 commits ahead of `origin/main` (never pushed there). If the API 502s again after an env change, check the deployment `meta.repo`/`meta.commitHash` first (it silently fell back to the stale repo before). API_KEY for hosted curl tests lives in the `intelligence-api` Railway env var (the `.env` has two API_KEY lines — the second, line 59, is the live one; line 5 is a base64 sample token).
